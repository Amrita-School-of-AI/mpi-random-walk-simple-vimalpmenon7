...
void walker_process()
{
    ...
    for (int step = 1; step <= max_steps; ++step)
    {
        ...
        if (posn < -domain_size || posn > domain_size)
        {
...
void controller_process()
{
    ...
    for (int i = 0; i < num_walkers; ++i)
    {
        int rsteps;
        MPI_Status status;
        MPI_Recv(&rsteps, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &status);
    }

    std::cout << "Controller: All " << no_walkers << " walkers have finished.\n";
}
